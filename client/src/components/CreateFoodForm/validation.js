...
export function validation(inputs) {
    ...
    if (!regexImage.test(image) || !image) errors.image = "la url de la imagen no es valida";
    if (!description || !regexAnyLetter.test(description)) errors.description = "el resumen no puede estar vacio";
    if (!steps || !regexAnyLetter.test(steps)) errors.steps = "deben existir pasos a seguir";
